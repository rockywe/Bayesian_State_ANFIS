...
try:
    ...
    print("错误：数据文件 './脱硫数据整理2.xlsx' 未找到，请检查路径。")
    exit()
...
def plot_detailed_scatter_plots(detailed_results, save_dir):
    ...
    for i, result in enumerate(detailed_results):
        ...
        # 绘制散点图
        ax.scatter(y_train_orig, y_pred_train, alpha=0.9, color='royalblue', s=110, edgecolor='black', label='Train Set')
        ax.scatter(y_test_orig, y_pred_test, alpha=0.9, color='darkorange', s=150, edgecolor='black', label='Test Set')
        ...
        # 添加性能指标文本框 (训练集和测试集并排显示)
        textstr = (
            rf'$\mathrm{{R}}^2_{{\mathrm{{train}}}} = {train_r2:.3f}$' + '\n' +
            rf'$\mathrm{{R}}^2_{{\mathrm{{test}}}} = {test_r2:.3f}$' + '\n' +
            rf'$\mathrm{{RMSE}}_{{\mathrm{{train}}}} = {train_rmse:.3f}\,\mathrm{{ppm}}$' + '\n' +
            rf'$\mathrm{{RMSE}}_{{\mathrm{{test}}}} = {test_rmse:.3f}\,\mathrm{{ppm}}$'
        )
        ...
        # 在循环中使用索引
        ax.set_title(f'{letters[i]}. {result["model_name"]}', fontsize=22, fontweight='bold', pad=10)
    ...
    plt.savefig(os.path.join(save_dir, f'prediction_comparison_{timestamp}.png'), dpi=600, bbox_inches='tight')
    plt.savefig(os.path.join(save_dir, f'prediction_comparison_{timestamp}.pdf'), bbox_inches='tight')
    print(f"详细散点对比图已保存到: {save_dir}")
    plt.show()
...
def run_comparison_experiment():
    ...
    optimizer_ann = torch.optim.SGD(ann_model.parameters(), lr=0.001)
    ...
    detailed_results_list.append(
        {'model_name': 'Multi-Layer Perceptron (MLP)', 'y_train_original': y_train_orig, 'y_pred_train': y_pred_train_ann,
    ...
    detailed_results_list.append(
        {'model_name': 'Baysian State-ANFIS', 'y_train_original': y_train_orig, 'y_pred_train': y_pred_train_bsanfis,
