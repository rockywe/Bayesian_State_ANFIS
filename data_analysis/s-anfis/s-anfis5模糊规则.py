...
def plot_comparison_results(results_list, save_dir):
    ...
    colors = plt.cm.tab10(np.linspace(0, 1, n_memb_funcs_per_input))
...
def plot_best_membership_evolution(best_result, save_dir):
    ...
    for i in range(n_inputs):
        ...
        for j in range(n_funcs):
            ...
            ax.plot(x_range, membership_init, '--', color=color, linewidth=1.5, alpha=0.95)

            # 训练后的隶属函数 (实线)
            membership_final = np.exp(-0.5 * ((x_range - mu_final[j]) / (sigma_final[j] + 1e-8)) ** 2)
            ax.plot(x_range, membership_final, '-', color=color, linewidth=2, alpha=0.95)
        ...
        ax.legend(handles=legend_handles, labels=legend_labels, handler_map={plt.Line2D: HandlerDashedSolidLine()},
                  fontsize=13, ncol=len(legend_labels), loc='upper center', columnspacing=1.4, handletextpad=0.6)
    ...
    if len(training_history['epochs']) > 1:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'parameter_trends_{optimizer_name}_{timestamp}.pdf'),
                    dpi=300, bbox_inches='tight')
