...
def plot_loss_curve(results_list, save_dir):
    """
    专门绘制单个优化器（RMSprop）的训练和验证损失曲线。
    """
    # 1. 筛选出 'rmsprop' 的结果
    # 使用 .lower() 进行不区分大小写的比较，更加稳健
    rmsprop_result = None
    for r in results_list:
        if r['optimizer_name'].lower() == 'rmsprop':
            rmsprop_result = r
            break  # 找到后就停止搜索

    # 如果没有找到rmsprop的结果，则打印信息并退出
    if rmsprop_result is None:
        print("在结果中未找到 'rmsprop' 优化器，不进行绘图。")
        return
    ...
    optimizer_name = rmsprop_result['optimizer_name']

    # 3. 开始绘图
    fig, ax = plt.subplots(figsize=(10, 7))  # 创建一个图和一个坐标轴

    # 绘制训练损失曲线
    ax.plot(epochs, train_losses, label='Train Loss', color='#D13C29', linewidth=3, alpha=0.8)

    # 绘制验证损失曲线
    ax.plot(epochs, valid_losses, label='Validation Loss', color='#5381B2', linewidth=3, alpha=0.8)

    # 4. 美化和设置图形
    # 借用您原来代码中优秀的样式设置
    ax.set_xlabel('Epochs', fontsize=14)
    ax.set_ylabel('Loss', fontsize=14)
    ax.set_yscale('log')  # 使用对数坐标轴，能更好地观察损失下降过程
    ax.legend(fontsize=12)
    ax.grid(False)  # 添加网格线，方便观察
    ...
    save_path_png = os.path.join(save_dir, f'rmsprop_loss_curve_{timestamp}.png')
    plt.savefig(save_path_png, dpi=300, bbox_inches='tight')

    plt.savefig(os.path.join(save_dir, f'rmsprop_loss_curve_{timestamp}.pdf'),
                bbox_inches='tight')
    plt.savefig(os.path.join(save_dir, f'rmsprop_loss_curve_{timestamp}.png'),
                dpi=600, bbox_inches='tight')


    print(f"损失曲线图已保存至: {save_path_png}")
    plt.show()
...
def plot_best_membership_evolution(best_result, save_dir):
    ...
    letters = ['$Pressure_{\mathrm{liquid}}\ (\mathrm{Standardization})$',
               '$Flow_{\mathrm{gas}}\ (\mathrm{Standardization})$',
               '$Temperature_{\mathrm{gas}}\ (\mathrm{Standardization})$',
               '$c_{\mathrm{H}_2\mathrm{S}}\ (\mathrm{Standardization})$',
    ...
    for i in range(n_inputs):
        ...
        ax.legend(handles=legend_handles, labels=legend_labels, handler_map={plt.Line2D: HandlerDashedSolidLine()},
    ...
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    plt.savefig(os.path.join(save_dir, f'membership_evolution_{optimizer_name}_{timestamp}.png'), dpi=600,
                bbox_inches='tight')
...
def run_optimizer_comparison():
    ...
    # 生成对比图表
    print(f"\n生成对比图表...")
    plot_loss_curve(results_list, exp_dir)
    #
    # # 绘制最佳优化器的隶属函数演化
