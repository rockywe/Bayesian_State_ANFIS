...
def plot_comparison_results(results_list, save_dir):
    ...
    # 训练损失对比
    colors = ['#D13C29', '#5381B2', '#66A655', '#8B549D', '#EA9839', '#9C9C9C']
    for i, result in enumerate(results_list):
        history = result['training_history']
        ax3.plot(history['epochs'], history['train_losses'],
                 label=f"{result['optimizer_name']}",
                 color=colors[i], linewidth=3, alpha=0.8)
    ax3.tick_params(axis='both', which='major', direction='out', length=6, color='black', labelsize=16, bottom=True, top=False, left=True, right=False)
    ax3.set_xlabel('Epochs', fontsize=20)
    ax3.set_ylabel('Train Loss', fontsize=20)
    ...
    for i, result in enumerate(results_list):
        ...
        ax4.plot(history['epochs'], history['valid_losses'],
                 label=f"{result['optimizer_name']}",
                 color=colors[i], linewidth=3, alpha=0.8)
    ax4.tick_params(axis='both', which='major', direction='out', length=6, color='black', labelsize=16, bottom=True, top=False, left=True, right=False)
    ax4.set_xlabel('Epochs', fontsize=20)
    ax4.set_ylabel('Valid Loss', fontsize=20)
...
def plot_best_membership_evolution(best_result, save_dir):
    ...
    axes = axes.ravel()  # 将 3x2 的子图数组扁平化为一维数组，方便索引
    ...
    for i in range(n_inputs):
        ...
        ax.set_title(f'{x_features[i]}的隶属函数变化', fontsize=12, fontweight='bold')
