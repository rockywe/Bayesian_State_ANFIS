...
def plot_control_comparison(comparison_results):
    ...
    fig.suptitle('模糊监督PID vs 传统PID 控制对比', fontsize=16, fontweight='bold')

    # 1. H2S出口浓度跟踪对比
    ax1 = axes[0, 0]
    ax1.plot(results['fuzzy_pid']['time_steps'], results['fuzzy_pid']['predicted_outputs'],
             'b-', linewidth=2, label='模糊监督PID')
    ax1.plot(results['traditional_pid']['time_steps'], results['traditional_pid']['predicted_outputs'],
             'g-', linewidth=2, label='传统PID')
    ax1.axhline(y=results['setpoint'], color='r', linestyle='--',
                linewidth=2, label=f'目标值 ({results["setpoint"]:.2f})')
    ax1.set_xlabel('时间步')
    ax1.set_ylabel('H2S出口浓度 (mg/m³)')
    ax1.set_title('H2S出口浓度跟踪对比')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    # 2. 控制误差对比
    ax2 = axes[0, 1]
    ax2.plot(results['fuzzy_pid']['time_steps'], np.abs(results['fuzzy_pid']['errors']),
             'b-', linewidth=2, label='模糊监督PID')
    ax2.plot(results['traditional_pid']['time_steps'], np.abs(results['traditional_pid']['errors']),
             'g-', linewidth=2, label='传统PID')
    ax2.set_xlabel('时间步')
    ax2.set_ylabel('绝对误差 (mg/m³)')
    ax2.set_title('控制误差对比')
    ax2.legend()
    ax2.grid(True, alpha=0.3)

    # 3. 转速控制对比
    ax3 = axes[1, 0]
    ax3.plot(results['fuzzy_pid']['time_steps'], results['fuzzy_pid']['rotation_speeds'],
             'b-', linewidth=2, label='模糊监督PID')
    ax3.plot(results['traditional_pid']['time_steps'], results['traditional_pid']['rotation_speeds'],
             'g-', linewidth=2, label='传统PID')
    ax3.set_xlabel('时间步')
    ax3.set_ylabel('转速 (rpm)')
    ax3.set_title('转速控制对比')
    ax3.legend()
    ax3.grid(True, alpha=0.3)

    # 4. PID输出对比
    ax4 = axes[1, 1]
    ax4.plot(results['fuzzy_pid']['time_steps'], results['fuzzy_pid']['pid_outputs'],
             'b-', linewidth=2, label='模糊监督PID')
    ax4.plot(results['traditional_pid']['time_steps'], results['traditional_pid']['pid_outputs'],
             'g-', linewidth=2, label='传统PID')
    ax4.axhline(y=0, color='k', linestyle='-', alpha=0.3)
    ax4.set_xlabel('时间步')
    ax4.set_ylabel('PID输出')
    ax4.set_title('PID输出对比')
    ax4.legend()
    ax4.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('./pic/pid_control_comparison.png',
                format='png', bbox_inches='tight', dpi=300)
    plt.close()

    print("PID控制对比结果图已保存到 ./pic/pid_control_comparison.png")
...
def main():
    ...
    # 保存结果
    # results_file = f'./pic/pid_comparison_results_{timestamp}.pkl'
