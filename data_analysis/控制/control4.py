...
def calculate_outlet_h2s(
    ...
    try:
        ...
        outlet_ppm = y_out * 1e6 / 40.0 # Keep the original factor
...
class PIDController:
    ...
    def compute(self, current_value, current_time):
        ...
        # PID output
        output = p_term + self.config.Ki * self.integral + d_term

        # Output clamping
        clamped_output = max(min_limit, min(output, max_limit))

        self.prev_error = error # Store error for next iteration
        self._output = clamped_output # Store clamped output
...
class FuzzyController:
    ...
    def __init__(self, setpoint, output_limits):
        ...
        # Adjustment in RPM (dRPM_adj) universe and MFs (singleton outputs)
        self.dRPM_adj_universe = (-100, 100) # Universe is for reference, singletons are points
        self.dRPM_adj_mf = {'NB': -90, 'NS': -40, 'Z': 0, 'PS': 40, 'PB': 90}

        # Fuzzy Rules: (Error, Change in Error) -> Adjustment in RPM
        self.rules = {
    ...
    def _defuzzify(self, rule_strengths):
        ...
        if not rule_strengths:
            return 0.0 # No rules fired, return zero adjustment
        ...
        for term, strength in rule_strengths.items():
            if term in self.dRPM_adj_mf:
                ...
                singleton_value = self.dRPM_adj_mf[term]
    ...
    def compute(self, current_value, current_time):
        """Computes the fuzzy control output (RPM adjustment)."""
        ...
        clamped_dRPM_adj = max(min_limit, min(dRPM_adj, max_limit))

        # Apply filtering (simple moving average or similar)
        # This smoothing helps prevent jerky control actions
        filtered_dRPM_adj = 0.9 * clamped_dRPM_adj + 0.1 * self.prev_dRPM_adj # Simple first-order filter
        self.prev_dRPM_adj = filtered_dRPM_adj # Store filtered value for next iteration

        return filtered_dRPM_adj # Return filtered adjustment

    def reset(self):
        """Resets the controller state."""
        self.prev_error = 0.0
        self.last_time = None
        self.prev_dRPM_adj = 0.0 # Reset filter state
...
def run_prediction_simulation(initial_params, target_h2s, rpm_control_type, duration_s=600, dt_s=5,
    ...
    if rpm_control_type == 'pid':
        ...
        # Fuzzy controller output limits for RPM adjustment
    ...
    try:
        ...
        for t_step in range(dt_s, duration_s + dt_s, dt_s):
            ...
            # RPM adjustment based on selected controller
            adj_RPM = rpm_controller.compute(current_conc, current_time_sim)

            # Apply adjustments and constraints for the *next* state
            # Ensure flow and RPM remain non-negative and within reasonable operating ranges
            process_params["脱硫液流量_m3h"] = max(0.0, min(process_params["脱硫液流量_m3h"] + adj_L, 60.0)) # Example max flow
            process_params["转速_RPM"] = max(0.0, min(process_params["转速_RPM"] + adj_RPM, 900.0)) # Example max RPM
        ...
        if control_mode in ['pid_next', 'fuzzy_next', 'opt_next']:
            ...
            if control_mode == 'pid_next':
                ...
                self.update_log("运行 Fuzzy 下一步预测...")
                 ...
                 recommended_rpm = min(900.0, max(700.0, current_rpm + (target_h2s - current_h2s) * -5)) # Simple heuristic
                 adj_RPM = recommended_rpm - current_rpm
                 ...
                 next_rpm = max(0.0, min(current_rpm + adj_RPM, 900.0))
            ...
            # Compute adjustments for PID/Fuzzy next step
            ...
            adj_RPM = rpm_controller.compute(current_h2s, current_time_sim)

            # Calculate next state based on adjustments
            next_flow = max(0.0, min(current_flow + adj_L, 60.0)) # Apply flow constraints
            next_rpm = max(0.0, min(current_rpm + adj_RPM, 900.0)) # Apply RPM constraints
