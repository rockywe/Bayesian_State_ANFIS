...
D_h2s = 1.8e-9  # H2S在溶液中的扩散系数，单位为 m^2/s
g_flow = 2 / 3600  # 气体流量，单位为立方米每秒

# Variables
rpm = 1200  # 转速
l_flow = 20e-3 / 3600  # 液体流量
tannic_coc = 0.4  # 单宁酸浓度，单位为 mol/L
sodium_coc = 0.8 # 偏钒酸钠的浓度
ph = 7.0  # 假设中性pH值

# 表观速率常数
k_tannic = 0.05  # 单宁酸的表观速率常数
k_sodium = 0.2  # 偏钒酸钠的表观速率常数
k_apparent = tannic_coc * k_tannic + sodium_coc * k_sodium
...
# 液膜传质系数
def calculate_transfer_l(D_h2s, k_apparent, t_new):
    transfer_l = (np.sqrt(D_h2s * k_apparent) / t_new) * (
...
transfer_l = calculate_transfer_l(D_h2s, k_apparent, t_new)
