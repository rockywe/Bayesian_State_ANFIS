...
class FuzzyController:
    def __init__(self, setpoint, output_limits):
        ...
        # Further increased output universe and singleton values for stronger control
        self.dRPM_adj_universe = (-100, 100) # Increased potential adjustment range
        self.dRPM_adj_mf = {
            'NB': -90, 'NS': -40, 'Z': 0, 'PS': 40, 'PB': 90 # Further increased singleton values
        }

        # Fuzzy Rules (Mamdani-like, simplified to singletons for output)
        # Slightly modified rules for potentially more aggressive response in certain conditions
        self.rules = {
            ('NB', 'NB'): 'PB', ('NB', 'NS'): 'PB', ('NB', 'Z'): 'PB', ('NB', 'PS'): 'PS',  ('NB', 'PB'): 'Z', # More aggressive for large negative error
            ('NS', 'NB'): 'PB', ('NS', 'NS'): 'PS', ('NS', 'Z'): 'PS',  ('NS', 'PS'): 'Z',  ('NS', 'PB'): 'NS',
            ('Z',  'NB'): 'PS', ('Z', 'NS'): 'Z',  ('Z', 'Z'): 'Z',  ('Z', 'PS'): 'Z',  ('Z', 'PB'): 'NS',
            ('PS', 'NB'): 'NS', ('PS', 'NS'): 'Z',  ('PS', 'Z'): 'NS', ('PS', 'PS'): 'NB',  ('PS', 'PB'): 'NB',
            ('PB', 'NB'): 'Z',  ('PB', 'NS'): 'NB',  ('PB', 'Z'): 'NB', ('PB', 'PS'): 'NB',  ('PB', 'PB'): 'NB' # More aggressive for large positive error
        }
    ...
    def compute(self, current_value, current_time):
        ...
        filtered_dRPM_adj = 0.8 * dRPM_adj + 0.2 * self.prev_dRPM_adj # Increased current value weight significantly
...
class ControlSystem:
    ...
    def run_simulation(self, duration=600, dt=5, control_type='PID'):
        ...
        # Ensure Fuzzy controller is re-initialized with updated limits
    ...
    def _adjust_parameters(self, t, current_conc, control_type):
        ...
        if control_type == 'PID':
            ...
            # Further reduced filtering for a more direct response
            adj_RPM = 0.9 * adj_RPM + 0.1 * prev_adj['RPM']
        ...
        self.process_params["转速_RPM"] = max(0.0, min(
            self.process_params["转速_RPM"] + adj_RPM, 900.0)) # New range 0-900
    ...
    def visualize(self):
        ...
        colors = ['#2c3e50', '#e74c3c', '#27ae60', '#2980b9', '#8e44ad']
        plt.rcParams.update({
            'font.size': 11,
            'axes.titlesize': 13,
            'axes.labelsize': 11,
        ...
        plt.xlabel('时间 (秒)')
        plt.ylabel('转速 (RPM)')
        plt.ylim(0, 950) # Adjusted y-limit for new RPM range (0-900)
        plt.grid(True, alpha=0.4)
        plt.legend()
        plt.tight_layout()
        ...
        plt.xlabel('时间 (秒)')
        plt.ylabel('流量 (m³/h)')
        plt.ylim(0, 65) # Adjusted y-limit for new Liquid Flow range (0-60)
        plt.grid(True, alpha=0.4)
        plt.legend()
        plt.tight_layout()
        ...
        plt.xlabel('时间 (秒)')
        plt.ylabel('流量 (m³/h)')
        plt.ylim(0, 3100) # Adjusted y-limit for new Gas Flow range (0-3000)
        plt.grid(True, alpha=0.4)
        plt.legend()
        plt.tight_layout()
