...
# 方法一：全局设置（优先推荐）
mpl.rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei'] # 尝试多种字体
...
class PIDController:
    ...
    def compute(self, current_value, current_time):
        ...
        if dt <= 0:
            ...
            self.prev_error = self.config.setpoint - current_value
            return 0.0
        ...
        if output > max_limit:
            output = max_limit
            # Only update integral if output is not saturated (basic anti-windup)
            # A more sophisticated approach would check the sign of error and saturation
            # if error * (output - max_limit) < 0: pass
            # else: self.integral = new_integral
        elif output < min_limit:
            output = min_limit
            # if error * (output - min_limit) < 0: pass
            # else: self.integral = new_integral
        else:
...
class FuzzyController:
    def __init__(self, setpoint, output_limits):
        ...
        self.e_universe = (-150, 150) # Example range, tune based on expected error
        self.e_mf = {
            'NB': lambda e: self._trimf(e, -150, -150, -50),
            'NS': lambda e: self._trimf(e, -100, -50, 0),
            'Z':  lambda e: self._trimf(e, -20, 0, 20),
            'PS': lambda e: self._trimf(e, 0, 50, 100),
            'PB': lambda e: self._trimf(e, 50, 150, 150)
        }

        # Change in Error (de): current error - previous error
        self.de_universe = (-50, 50) # Example range, tune based on expected change
        self.de_mf = {
            'NB': lambda de: self._trimf(de, -50, -50, -15),
            'NS': lambda de: self._trimf(de, -30, -15, 0),
            'Z':  lambda de: self._trimf(de, -5, 0, 5),
            'PS': lambda de: self._trimf(de, 0, 15, 30),
            'PB': lambda de: self._trimf(de, 15, 50, 50)
        }

        # Output (dRPM_adj): Change in RPM adjustment
        self.dRPM_adj_universe = (-30, 30) # Example range, tune based on desired adjustment
        self.dRPM_adj_mf = {
            'NB': -25, 'NS': -10, 'Z': 0, 'PS': 10, 'PB': 25
        }
        ...
        self.rules = {
            ...
            ('NS', 'NB'): 'PB', ('NS', 'NS'): 'PB', ('NS', 'Z'): 'PS', ('NS', 'PS'): 'Z',  ('NS', 'PB'): 'NS',
            ('Z',  'NB'): 'PB', ('Z', 'NS'): 'PS',  ('Z', 'Z'): 'Z',  ('Z', 'PS'): 'NS',  ('Z', 'PB'): 'NB',
            ('PS', 'NB'): 'PS', ('PS', 'NS'): 'Z',  ('PS', 'Z'): 'NS', ('PS', 'PS'): 'NB',  ('PS', 'PB'): 'NB',
...
def calculate_outlet_h2s(
    ...
    try:
        ...
        NTU = max(0.0, min(NTU, 10.0)) # Cap NTU to prevent math errors with exp(-NTU)
...
class ControlSystem:
    def __init__(self):
        ...
        self.pid_RPM = PIDController(PIDConfig(
            ...
            setpoint=50, # Target H2S concentration
    ...
    def run_simulation(self, duration=600, dt=5, control_type='PID'):
        ...
        if control_type == 'PID':
            self.history_pid = self._init_history()
            self.pid_L = PIDController(PIDConfig(Kp=-0.25, Ki=-0.001, Kd=-0.015, setpoint=50, output_limits=(-1.5, 1.5)))
            self.pid_G = PIDController(PIDConfig(Kp=0.01, Ki=0.0003, Kd=0.002, setpoint=50, output_limits=(-3, 3)))
            self.pid_RPM = PIDController(PIDConfig(Kp=-0.8, Ki=-0.03, Kd=-0.6, setpoint=50, output_limits=(-20, 20)))
            self.prev_adj_pid = {'L': 0, 'RPM': 0, 'G': 0}
        elif control_type == 'Fuzzy':
            self.history_fuzzy = self._init_history()
            self.pid_L = PIDController(PIDConfig(Kp=-0.25, Ki=-0.001, Kd=-0.015, setpoint=50, output_limits=(-1.5, 1.5)))
            self.pid_G = PIDController(PIDConfig(Kp=0.01, Ki=0.0003, Kd=0.002, setpoint=50, output_limits=(-3, 3)))
            self.fuzzy_RPM = FuzzyController(setpoint=50, output_limits=(-20, 20))
            self.prev_adj_fuzzy = {'L': 0, 'RPM': 0, 'G': 0}
        else:
    ...
    def visualize(self):
        ...
        # plt.axhline(self.pid_L.config.setpoint, color=colors[2], linestyle='--', linewidth=1.5, label='设定值') # Use a different color for setpoint
...
if __name__ == "__main__":
    ...
    system.run_simulation(duration=1000, control_type='PID')
    system.run_simulation(duration=1000, control_type='Fuzzy')
    system.visualize()
